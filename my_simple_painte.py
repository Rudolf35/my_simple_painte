...
def free_drawing(canvas_width=640, canvas_height=480, init_brush_radius=3):
    ...
    while True:
        ...
        if key == 27: # ESC
            ...
            center = mouse_xy
            cv.circle(canvas, center, radius=60, color=palette[brush_color], thickness=brush_radius)
        elif key == ord('w'):
            xsize, ysize = 60, 50
            pt1, pt2 = (mouse_xy[0] - xsize, mouse_xy[1] - ysize), (mouse_xy[0] + xsize, mouse_xy[1] + ysize)
            cv.rectangle(canvas, pt1, pt2, color=palette[brush_color], thickness=brush_radius)
        elif key == ord('e'):
            pts = np.array([(mouse_xy[0], mouse_xy[1]-50), (mouse_xy[0]-55, mouse_xy[1]+50), (mouse_xy[0]+55, mouse_xy[1]+50)])
            cv.polylines(canvas, [pts], True, color=palette[brush_color], thickness=brush_radius)
